...
多计算节点集群架构和完整硬件虚拟化特性，方便用户进行持续集成和持续部署（CI/CD），是加速构建 RISC-V 软件生态的有效工具。在向量扩展基础上，Cluster Server 支持每秒两千八百亿次（2880 TOPS）通用 AI 运算和 1536 GB 容量内存，满足多路并发请求的 AI 推理任务。
