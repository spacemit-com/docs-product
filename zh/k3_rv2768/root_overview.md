...
RVA23 针对通用计算领域的应用处理器所制定，要求支持向量（Vector）和虚拟化（Hypervisor）扩展，是面向 AI 和高性能工作负载的现代性能标准。Cluster Server 提供 768 核 RISC-V 原生算力，适合处理大量应用程序和大型操作系统等编译任务。
多计算节点集群架构和完整硬件虚拟化特性，方便用户进行持续集成和持续部署（CI/CD），是加速构建 RISC-V 软件生态的有效工具。在向量扩展基础上，Cluster Server 支持每秒两千八百亿次（2880 TOPS）通用 AI 运算和 1536 GB 容量内存，可部署 800 亿参数（80B）混合专家模型，满足 8～16 路并发请求的 AI 推理任务。
...
| **交换系统** | 外部端口 | 对外支持 2 × 10GE SFP28 光口和 48 × 10GE SFP+ 光口 |
