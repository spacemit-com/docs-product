...
K3 CoM260 开发套件的 Type-C 连接器仅支持 OTG 模式，不支持供电。
...
K3 CoM260 采用 DCIN 供电。建议使用符合相关质量认证、输出规格正确的 DC JACK 电源适配器，且其最大功率不低于 35 W。
...
分别使用按键将 FC_REC 与 GND、SYS_RST 与 GND 连接，先按住 FC_REC 按键，再按下 RST 按键，即可进入刷机模式。
